...
for i in `seq "${capi}" "${capf}"`; do
    ...
    while true; do
        ...
        if false; then
            ...
            ( mplayer -vc dummy -vo null -af resample=44100:0:0 -af volume=0:0 -ao pcm:file="${arq}.wav" -chapter "${i}-${i}" -dvd-device "${dispositivo}" "dvd://${titulo}" || true ) && lame -b 320 "${arq}.wav" "${arq}.mp3" && rm -f "${arq}.wav" && ajusteganho "${arq}.mp3"
        fi
        if [ ${?} -eq 0 ]; then
            break
        else
